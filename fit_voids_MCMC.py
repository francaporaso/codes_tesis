...
def log_likelihood_DSt_hamaus(theta, r, y, yerr):
    ...
    '''
    rs,dc,a,b = theta
    modelo = DSt_hamaus_paralelo(r, rs, dc, a, b)
...
def log_likelihood_h2DS(theta, r, y, yerr):
    dc,b = theta
    modelo = h2DS(r,dc,b)
...
def log_prior_h2DS(theta):
    dc,b = theta
    if (-1. <= dc <= 0.)&(1. <= b <= 20.):
        return 0.0
    return -np.inf

def log_probability_h2DS(theta, r, y, yerr):
    lp = log_prior_h2DS(theta)
    if not np.isfinite(lp):
        return -np.inf
    return lp + log_likelihood_h2DS(theta, r, y, yerr)
...
if __name__ == '__main__':
    ...
    for j,carpeta in enumerate(['Rv_6-10','Rv_10-50']):
        ...
        for k, archivo in enumerate(['all','R','S']):
            ...
            with fits.open(f'/home/fcaporaso/profiles/voids/{carpeta}/lensing_Rv{carpeta.split("_")[1]}_z02-04_type{archivo}_RMAX5.fits') as dat:
               h = dat[0].header
